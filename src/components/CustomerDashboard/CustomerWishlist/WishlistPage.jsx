...
const WishlistPage = () => {
  ...
  const getProductImage = (productImages) => {
    if (!productImages) return 'https://images.pexels.com/photos/533280/pexels-photo-533280.jpeg?auto=compress&cs=tinysrgb&w=300&h=200&fit=crop';
    
    try {
      const images = JSON.parse(productImages);
      return images.length > 0 ? images[0] : 'https://images.pexels.com/photos/533280/pexels-photo-533280.jpeg?auto=compress&cs=tinysrgb&w=300&h=200&fit=crop';
    } catch (error) {
      return 'https://images.pexels.com/photos/533280/pexels-photo-533280.jpeg?auto=compress&cs=tinysrgb&w=300&h=200&fit=crop';
    }
  };

  // Check if product is in stock (you can customize this logic)
  const isInStock = (item) => {
    // For now, we'll assume all products are in stock
    // You can add stock checking logic here based on your requirements
    return true;
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 max-w-6xl mx-auto">
      {/* Toast Notification */}
      {toast.show && (
        <div className={`mb-4 px-4 py-3 rounded text-center text-white ${toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}>
          {toast.message}
        </div>
      )}
      ...
      {loading ? (
        <div className="text-center py-16">
          <Loader2 size={64} className="text-green-600 mx-auto mb-4 animate-spin" />
          <h3 className="text-xl font-semibold text-gray-600 mb-2">Loading your wishlist...</h3>
          <p className="text-gray-500">Please wait while we fetch your saved items</p>
        </div>
      ) : wishlist.length === 0 ? (
        <div className="text-center py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-20 mt-10">
          {wishlist.map((item) => {
            ...
            const discount = calculateDiscount(parseFloat(item.price), parseFloat(item.price) * 1.2); // Assuming 20% markup for original price
            ...
            return (
              <div key={item.id} className="border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 group">
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-600 mb-3">{item.category || 'Product'}</p>
