...
export const WishlistProvider = ({ children }) => {
  ...
  const addToWishlist = async (productId) => {
    ...
    if (user && user.role === 'customer') {
      // Logged-in customer
      try {
        const response = await axios.post('http://localhost/backend/add_to_wishlist.php', {
          productId: productId,
          customerId: user.id
        });
  ...
  const removeFromWishlist = async (productId) => {
    ...
    if (user && user.role === 'customer') {
      // Logged-in customer
      try {
        const response = await axios.post('http://localhost/backend/remove_from_wishlist.php', {
          productId: productId,
          customerId: user.id
        });
  ...
  const loadWishlist = useCallback(async () => {
    ...
    if (user && user.role === 'customer') {
      try {
        setLoading(true);
        console.log('WishlistContext: Loading wishlist for customer ID:', user.id);
        ...
        const responsePromise = axios.post('http://localhost/backend/get_wishlist.php', {
          customerId: user.id
        });
        ...
        if (response.data.success) {
          ...
          console.log('WishlistContext: Wishlist loaded successfully:', response.data.wishlist);
        } else {
          console.error('WishlistContext: API returned error:', response.data.message);
          setWishlist([]);
          setWishlistCount(0);
        }
      } catch (error) {
        console.error('WishlistContext: Error loading wishlist:', error);
        console.error('WishlistContext: Error details:', error.response?.data || error.message);
        setWishlist([]);
        setWishlistCount(0);
      } finally {
        setLoading(false);
      ...
      console.log('WishlistContext: Guest user, wishlist count:', guestWishlist.length);
      setLoading(false);
    }
  }, []);
